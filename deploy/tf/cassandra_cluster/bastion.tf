...
locals {
  bastion_provisioner_static_vars = "WEBAPI_GOMEMLIMIT_GB=${local.webapi_gomemlimit_gb} WEBAPI_GOGC=${var.webapi_gogc} AWSREGION=${var.awsregion} SSH_USER=${var.ssh_user} OS_ARCH=${var.os_arch} CAPILLARIES_RELEASE_URL=${var.capillaries_release_url} EXTERNAL_WEBAPI_PORT=${var.external_webapi_port} INTERNAL_WEBAPI_PORT=${var.internal_webapi_port} EXTERNAL_ACTIVEMQ_CONSOLE_PORT=${var.external_activemq_console_port} EXTERNAL_PROMETHEUS_CONSOLE_PORT=${var.external_prometheus_console_port} BASTION_ALLOWED_IPS=${var.BASTION_ALLOWED_IPS} S3_LOG_URL=${var.s3_log_url} CASSANDRA_HOSTS=${local.cassandra_hosts} CASSANDRA_PORT=${var.cassandra_port} CASSANDRA_USERNAME=${var.cassandra_username} CASSANDRA_PASSWORD=${var.cassandra_password} ACTIVEMQ_URL=${local.activemq_url} ACTIVEMQ_USER_NAME=${var.activemq_user_name} ACTIVEMQ_USER_PASS=${var.activemq_user_pass} ACTIVEMQ_ADMIN_NAME=${var.activemq_admin_name} ACTIVEMQ_ADMIN_PASS=${var.activemq_admin_pass} ACTIVEMQ_SERVER_VERSION=${var.activemq_server_version} PROMETHEUS_NODE_EXPORTER_VERSION=${var.prometheus_node_exporter_version} PROMETHEUS_SERVER_VERSION=${var.prometheus_server_version} PROMETHEUS_NODE_TARGETS=${local.prometheus_node_targets} PROMETHEUS_JMX_TARGETS=${local.prometheus_jmx_targets} PROMETHEUS_GO_TARGETS=${local.prometheus_go_targets}"
}
