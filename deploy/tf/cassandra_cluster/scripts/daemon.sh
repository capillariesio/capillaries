...
echo GOMEMLIMIT="$DAEMON_GOMEMLIMIT_GB"GiB GOGC=$DAEMON_GOGC AWS_DEFAULT_REGION=$AWSREGION CAPI_CASSANDRA_HOSTS="$CASSANDRA_HOSTS" CAPI_CASSANDRA_PORT=$CASSANDRA_PORT CAPI_CASSANDRA_USERNAME=$CASSANDRA_USERNAME CAPI_CASSANDRA_PASSWORD=$CASSANDRA_PASSWORD CAPI_CASSANDRA_KEYSPACE_REPLICATION_CONFIG='"'"{'class':'NetworkTopologyStrategy','datacenter1':1}"'"' CAPI_CASSANDRA_CONSISTENCY=LOCAL_QUORUM CAPI_CASSANDRA_WRITER_WORKERS=$WRITER_WORKERS CAPI_THREAD_POOL_SIZE=$THREAD_POOL_SIZE CAPI_AMQP091_URL=$RABBITMQ_URL CAPI_CASSANDRA_TIMEOUT=15000 CAPI_PYCALC_INTERPRETER_PATH=python3 CAPI_LOG_LEVEL=info CAPI_DEAD_LETTER_TTL=5000 CAPI_LOG_FILE=/var/log/capillaries/capidaemon.log /home/$SSH_USER/bin/capidaemon "&>/dev/null &"
