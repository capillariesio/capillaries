...
if [ "$MAX_PARTITION_KEYS_IN_SELECT" = "" ]; then
  echo Error, missing: MAX_PARTITION_KEYS_IN_SELECT=5
  exit 1
fi
...
# Install Python 3.12 (3.13 consumes more memory and dies with OOM and takes capidaemon with it)

sudo DEBIAN_FRONTEND=noninteractive NEEDRESTART_MODE=a apt-get update -y
sudo add-apt-repository ppa:deadsnakes/ppa
sudo DEBIAN_FRONTEND=noninteractive NEEDRESTART_MODE=a apt-get update -y
sudo DEBIAN_FRONTEND=noninteractive NEEDRESTART_MODE=a apt-get install -y python3.12
...
echo GOMEMLIMIT="$DAEMON_GOMEMLIMIT_GB"GiB GOGC=$DAEMON_GOGC AWS_DEFAULT_REGION=$AWSREGION CAPI_PROMETHEUS_EXPORTER_PORT=$DAEMON_PROMETHEUS_EXPORTER_PORT CAPI_CASSANDRA_HOSTS="$CASSANDRA_HOSTS" CAPI_CASSANDRA_PORT=$CASSANDRA_PORT CAPI_CASSANDRA_USERNAME=$CASSANDRA_USERNAME CAPI_CASSANDRA_PASSWORD=$CASSANDRA_PASSWORD CAPI_CASSANDRA_KEYSPACE_REPLICATION_CONFIG='"'"{'class':'NetworkTopologyStrategy','datacenter1':1}"'"' CAPI_CASSANDRA_CONSISTENCY=LOCAL_QUORUM CAPI_CASSANDRA_WRITER_WORKERS=$WRITER_WORKERS CAPI_THREAD_POOL_SIZE=$THREAD_POOL_SIZE CAPI_MAX_PARTITION_KEYS_IN_SELECT=$MAX_PARTITION_KEYS_IN_SELECT CAPI_MQ_TYPE=$MQ_TYPE CAPI_CAPIMQ_CLIENT_URL=$CAPIMQ_CLIENT_URL CAPI_CAPIMQ_CLIENT_HEARTBEAT_INTERVAL=$CAPIMQ_CLIENT_HEARTBEAT_INTERVAL CAPI_AMQ10_URL=$AMQP10_URL CAPI_AMQP10_ADDRESS=$AMQP10_ADDRESS CAPI_AMQP10_RETRY_METHOD=$AMQP10_RETRY_METHOD CAPI_CASSANDRA_TIMEOUT=15000 CAPI_PYCALC_INTERPRETER_PATH=python3.12 CAPI_LOG_LEVEL=info CAPI_LOG_FILE=/var/log/capillaries/capidaemon.log /home/$SSH_USER/bin/capidaemon "&>/dev/null &"
GOMEMLIMIT="$DAEMON_GOMEMLIMIT_GB"GiB GOGC=$DAEMON_GOGC AWS_DEFAULT_REGION=$AWSREGION CAPI_PROMETHEUS_EXPORTER_PORT=$DAEMON_PROMETHEUS_EXPORTER_PORT CAPI_CASSANDRA_HOSTS="$CASSANDRA_HOSTS" CAPI_CASSANDRA_PORT=$CASSANDRA_PORT CAPI_CASSANDRA_USERNAME=$CASSANDRA_USERNAME CAPI_CASSANDRA_PASSWORD=$CASSANDRA_PASSWORD CAPI_CASSANDRA_KEYSPACE_REPLICATION_CONFIG="{'class':'NetworkTopologyStrategy','datacenter1':1}" CAPI_CASSANDRA_CONSISTENCY=LOCAL_QUORUM CAPI_CASSANDRA_WRITER_WORKERS=$WRITER_WORKERS CAPI_THREAD_POOL_SIZE=$THREAD_POOL_SIZE CAPI_MAX_PARTITION_KEYS_IN_SELECT=$MAX_PARTITION_KEYS_IN_SELECT CAPI_MQ_TYPE=$MQ_TYPE CAPI_CAPIMQ_CLIENT_URL=$CAPIMQ_CLIENT_URL CAPI_CAPIMQ_CLIENT_HEARTBEAT_INTERVAL=$CAPIMQ_CLIENT_HEARTBEAT_INTERVAL CAPI_AMQP10_URL=$AMQP10_URL CAPI_AMQP10_ADDRESS=$AMQP10_ADDRESS CAPI_AMQP10_RETRY_METHOD=$AMQP10_RETRY_METHOD CAPI_CASSANDRA_TIMEOUT=15000 CAPI_PYCALC_INTERPRETER_PATH=python3.12 CAPI_LOG_LEVEL=info CAPI_LOG_FILE="/var/log/capillaries/capidaemon.log" /home/$SSH_USER/bin/capidaemon &>/dev/null &
