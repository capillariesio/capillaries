...
for (( i=0; i<10; i++ )); do
    if sudo aws s3 cp s3://${capillaries_tf_deploy_temp_bucket_name}/cassandra.sh /tmp/; then
...
echo Running as ${ssh_user}: '${cassandra_provisioner_vars} /tmp/cassandra.sh > /tmp/cassandra.out 2>/tmp/cassandra.err'
