...
outDir=/mnt/capi_out/py_calc_quicktest
...
if ! diff -b $outDir/taxed_order_items_py_baseline.csv $outDir/taxed_order_items_py.csv; then
  echo -e "Python custom processor test diff \033[0;31mFAILED\e[0m"
  exit 1
else
  echo -e "Python custom processor test diff \033[0;32mOK\e[0m"
fi

if ! diff -b $outDir/taxed_order_items_go_baseline.csv $outDir/taxed_order_items_go.csv; then
  echo -e "Golang table-to-table test diff \033[0;31mFAILED\e[0m"
  exit 1
else
  echo -e "Golang table-to-table test diff \033[0;32mOK\e[0m"
fi