...
sshKeyFile=$(jq -r .ssh_config.private_key_path $1)
if [ "$sshKeyFile" = "" ]; then
  echo Error, missing ssh_config.private_key_path in $1
  exit 1
fi
externalIpAddress=$(jq -r .ssh_config.external_ip_address $1)
if [ "$externalIpAddress" = "" ]; then
  echo Error, missing ssh_config.external_ip_address in $1
  exit 1
fi

# Find string containing all cass ips: 10.5.0.11,10.5.0.12,10.5.0.13,10.5.0.14,10.5.0.15,10.5.0.16,10.5.0.17,10.5.0.18
cassIpString=$(jq -r .instances.bastion.service.env.CASSANDRA_HOSTS $1 | sed -e 's/[^0-9.,]//g')
...
$capideploy stop_services $cassNamesString -prj="$1"
...
do
  ...
  $capideploy config_services $cassNodeNickname -prj="$1"
  if [ "$?" -ne "0" ]; then
  ...
  do
    ...
    if [[ "$nodetoolOutput" == *"UJ  $cassNodeIp"* ]]; then
      echo $cassNodeNickname is joining UJ the cluster, almost there...
