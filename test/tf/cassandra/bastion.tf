...
locals {
  bastion_provisioner_vars = "AWSREGION=${var.awsregion} SSH_USER=${var.ssh_user} OS_ARCH=${var.os_arch} CAPILLARIES_RELEASE_URL=${var.capillaries_release_url} BASTION_EXTERNAL_IP_ADDRESS=${aws_eip.bastion_public_ip.public_ip} EXTERNAL_WEBAPI_PORT=${var.external_webapi_port} INTERNAL_WEBAPI_PORT=${var.internal_webapi_port} EXTERNAL_RABBITMQ_CONSOLE_PORT=${var.external_rabbitmq_console_port} EXTERNAL_PROMETHEUS_CONSOLE_PORT=${var.external_prometheus_console_port} BASTION_ALLOWED_IPS=${var.BASTION_ALLOWED_IPS} S3_LOG_URL=${var.s3_log_url} CASSANDRA_HOSTS=${local.cassandra_hosts} CASSANDRA_PORT=${var.cassandra_port} CASSANDRA_USERNAME=${var.cassandra_username} CASSANDRA_PASSWORD=${var.cassandra_password} RABBITMQ_URL=${local.rabbitmq_url} RABBITMQ_USER_NAME=${var.rabbitmq_user_name} RABBITMQ_USER_PASS=${var.rabbitmq_user_pass} RABBITMQ_ADMIN_NAME=${var.rabbitmq_admin_name} RABBITMQ_ADMIN_PASS=${var.rabbitmq_admin_pass} RABBITMQ_ERLANG_VERSION_AMD64=${var.rabbitmq_erlang_version_amd64} RABBITMQ_SERVER_VERSION_AMD64=${var.rabbitmq_server_version_amd64} RABBITMQ_ERLANG_VERSION_ARM64=${var.rabbitmq_erlang_version_arm64} RABBITMQ_SERVER_VERSION_ARM64=${var.rabbitmq_server_version_arm64} PROMETHEUS_NODE_EXPORTER_VERSION=${var.prometheus_node_exporter_version} PROMETHEUS_SERVER_VERSION=${var.prometheus_server_version} PROMETHEUS_TARGETS=${local.prometheus_targets}"
}
...
resource "aws_instance" "bastion" {
  ...
  user_data              = templatefile("./bastion.sh.tpl", {
    ...
    bastion_provisioner_vars               = local.bastion_provisioner_vars
...
output "output_bastion_provisioner_vars" {
  value = local.bastion_provisioner_vars
}