...
[Integration tests suite](./testing.md#integration-tests) contains tests that use data and config files stored in AWS S3. [Terraform deploy](../deploy/README.md) also use S3 files. You have to perform some AWS setup steps to make S3 integration tests work.

For demo purposes, Capillaries Docker-based test environment uses user-based permissions (see below), which may not be the preferred way of controlling access in production AWS environment. Using role assumption approach is the recommended way and [Capillaries Terraform scripts](../deploy/README.md) follow that approach.
...
daemon/webapi/toolbelt running in our Docker tests will this IAM user credentials and access `capillaries-testbucket`.
...
Now, everytime you run Capillaries test Docker setup (say, docker compose), make sure you have those variables set:

```
source ~/UserAccessCapillariesTestbucket.rc
```

docker-compose.yml uses those environment variables when creating containers for daemons and webapi.
