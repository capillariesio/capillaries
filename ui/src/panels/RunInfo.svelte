    ...
    <tbody>
        ...
        <tr>
            ...
            <td colspan="2"  style="white-space: nowrap;">
                ...
                {#if run_lifespan.final_status != 3}<button on:click={onStop} title={(run_lifespan.final_status === 1 ? "Stop run":"Invalidate the results of a complete run so they cannot be used in depending runs")}>{#if run_lifespan.final_status === 1}Stop{:else}Invalidate{/if}</button>{:else}&nbsp;{/if}</td>
        </tr>
    </tbody>
</table>
{/if}
