...
func (p *CapimqProducer) Send(wfmodelMsg *wfmodel.Message) error {
	...
	msgs := make([]*capimq.CapimqMessage, 1)
	msgs[0] = &capimq.CapimqMessage{Id: wfmodelMsg.Id, CapimqWaitRetryGroup: wfmodelMsg.FullNodeId()}
...
func (p *CapimqProducer) SendBulk(wfmodelMsgs []*wfmodel.Message) error {
	var marshalErr error
	msgs := make([]*capimq.CapimqMessage, len(wfmodelMsgs))
	for i, wfmodelMsg := range wfmodelMsgs {
		msgs[i] = &capimq.CapimqMessage{Id: wfmodelMsg.Id, CapimqWaitRetryGroup: wfmodelMsg.FullNodeId()}
