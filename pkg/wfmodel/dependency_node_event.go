...
func (e *DependencyNodeEvent) ToString() string {
	...
	fmt.Fprintf(&sb, "run_start_ts:%s,", e.RunStartTs.Format(LogTsFormatQuoted))
	fmt.Fprintf(&sb, "run_final_status:%s,", e.RunFinalStatus.ToString())
	fmt.Fprintf(&sb, "run_completed_ts:%s,", e.RunCompletedTs.Format(LogTsFormatQuoted))
	fmt.Fprintf(&sb, "run_stopped_ts:%s,", e.RunStoppedTs.Format(LogTsFormatQuoted))
	fmt.Fprintf(&sb, "node_is_started:%t,", e.NodeIsStarted)
	fmt.Fprintf(&sb, "node_start_ts:%s,", e.NodeStartTs.Format(LogTsFormatQuoted))
	fmt.Fprintf(&sb, "node_status:%s,", e.NodeStatus.ToString())
	fmt.Fprintf(&sb, "node_status_ts:%s", e.NodeStatusTs.Format(LogTsFormatQuoted))
	sb.WriteString("}")
	return sb.String()
}
