...
func TestReFunctions(t *testing.T) {
	...
	vars = eval.VarValuesMap{"r": map[string]any{"product_spec": `{"k":"Ideal For","v":"Boys, Men, Girls, Women"}`}}
	re = "re.MatchString(`\"k\":\"Ideal For\",\"v\":\"[\\w ,]*Boys[\\w ,]*\"`, r.product_spec)"
	assertEqual(t, re, true, vars)

	vars = eval.VarValuesMap{"r": map[string]any{"product_spec": `{"k":"Water Resistance Depth","v":"100 m"}`}}
	re = "re.MatchString(`\"k\":\"Water Resistance Depth\",\"v\":\"(100|200) m\"`, r.product_spec)"
	assertEqual(t, re, true, vars)

	vars = eval.VarValuesMap{"r": map[string]any{"product_spec": `{"k":"Occasion","v":"Ethnic, Casual, Party, Formal"}`}}
	re = "re.MatchString(`\"k\":\"Occasion\",\"v\":\"[\\w ,]*(Casual|Festive)[\\w ,]*\"`, r.product_spec)"
	assertEqual(t, re, true, vars)

	vars = eval.VarValuesMap{"r": map[string]any{"product_spec": `{"k":"Base Material","v":"Gold"},{"k":"Gemstone","v":"Diamond"}`, "retail_price": 101}}
	...
	vars = eval.VarValuesMap{"r": map[string]any{"product_spec": `{"k":"Base Material","v":"Gold"},{"k":"Gemstone","v":"Diamond"}`, "retail_price": 100}}
	re = "re.MatchString(`\"k\":\"Base Material\",\"v\":\"Gold\"`, r.product_spec) && re.MatchString(`\"k\":\"Gemstone\",\"v\":\"Diamond\"`, r.product_spec) && r.retail_price > 100"
