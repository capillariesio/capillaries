...
func TestCastSingle(t *testing.T) {
	...
	assert.Equal(t, decimal.NewFromFloat(12.0), val)
	val, _ = CastToDecimal(int32(12))
	assert.Equal(t, decimal.NewFromFloat(12.0), val)
	val, _ = CastToDecimal(float64(12.1))
	assert.Equal(t, decimal.NewFromFloat(12.1), val)
	val, _ = CastToDecimal(float32(12.1))
	assert.Equal(t, decimal.NewFromFloat(12.1), val)
	val, _ = CastToDecimal(float64(12.1))
	assert.Equal(t, decimal.NewFromFloat(12.1), val)
	val, _ = CastToDecimal(decimal.NewFromFloat(12.1))
	assert.Equal(t, decimal.NewFromFloat(12.1), val)
	_, err = CastToDecimal("12")
	assert.Equal(t, "cannot cast 12(string) to decimal, unsuported type", err.Error())
}
