...
func (node *ScriptNodeDef) evalCreatorAndLookupExpressionsAndCheckType() error {
	...
	if node.HasLookup() && node.Lookup.UsesFilter() {
		if err := evalExpressionWithFieldRefsAndCheckType(node.Lookup.Filter, node.Lookup.UsedInFilterFields, evalcapi.FieldTypeBool); err != nil {
	...
	if node.HasTableCreator() {
		// Having
		if err := evalExpressionWithFieldRefsAndCheckType(node.TableCreator.Having, node.TableCreator.UsedInHavingFields, evalcapi.FieldTypeBool); err != nil {
	...
	if node.HasFileCreator() {
		// Having
		if err := evalExpressionWithFieldRefsAndCheckType(node.FileCreator.Having, node.FileCreator.UsedInHavingFields, evalcapi.FieldTypeBool); err != nil {
