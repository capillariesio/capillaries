...
func TestDependencyPolicyBad(t *testing.T) {
	...
	conf = `{"event_priority_order": "run_id(asc)",	"rules": [{	"cmd": "go", "expression": "e.run_is_current && e.run_status == bad"	}]}`
	err = polDef.Deserialize([]byte(conf))
	assertErrorPrefix(t, "cannot parse rule expression 'e.run_is_current && e.run_status == bad': plain (non-selector) identifiers", err.Error())
}
...
func TestDependencyPolicyGood(t *testing.T) {
	...
	conf := `{"event_priority_order": "run_id(asc),run_is_current(desc)", "rules": [
		{"cmd": "go", "expression": "e.run_is_current && time.DiffMilli(e.run_start_ts, e.node_status_ts) > 0 && e.run_status == wfmodel.RunStart"	},
