...
func TestFunctions(t *testing.T) {
	...
	assertIterScan(t,
		`[[3 1 6 2 11.5 30 10 {{false [15]} 0} {{false [5]} 0} 3 1 6 2 3 1 6 2]]`, s,
		"SELECT max(b), min(b), sum(b), avg(b) , max(c), sum(c), avg(c), sum(d), avg(d),max(e), min(e), sum(e), avg(e),max(f), min(f), sum(f), avg(f) FROM ks1.t1")
...
func TestArithmeticCorrectness(t *testing.T) {
	...
	assertIterScan(t, "[[{{false [3333333333333333]} 16}]]", s, "SELECT avg(val) FROM ks1.t1 where bucket in (1, 2, 3)")
}
...
func TestAggregatesOverflow(t *testing.T) {
	...
	for _, i := range []int{1, 2, 3} {
		assertUpserMapScanCas(t, true, s, fmt.Sprintf("INSERT INTO ks1.t1 (bucket, v1, v2, v3, v4, v5) values (%d, %d, %d, %d, %d, %d)",
			i, math.MinInt8, math.MinInt16, math.MinInt32, math.MinInt64+1, math.MinInt64+1), existingRowMap) // TODO: figure out why it does not work without +1
	}
	result = fmt.Sprintf("[[%d %d %d %d %d]]", math.MinInt8, math.MinInt16, math.MinInt32, math.MinInt64+1, math.MinInt64+1)
	assertIterScan(t, result, s, "SELECT avg(v1), avg(v2), avg(v3), avg(v4), avg(v5) from ks1.t1 where bucket in (1, 2, 3)")
...
func TestDoubleAggregatesPrecision(t *testing.T) {
	...
	assertIterScan(t, "[[3.4028234663852886e+38 +Inf {{false [2 0 0 0 460104178379784192 3720971210472204916 13634728744079349334 4352831160636023503 2818437035441637264 10525621793288769572 16822849522073855861 13341554586340355996 15672481403480758088 6159439812001051182 7720983249868427763 18446744073709549484]} 0}]]", s, "SELECT avg(v1), avg(v2), avg(v3) from ks1.t1 where bucket in (1, 2, 3)")
	...
	assertIterScan(t, "[[110.11000066666666 110.11 {{false [11011]} 2}]]", s, "SELECT avg(v1), avg(v2), avg(v3) from ks1.t1 where bucket in (4,5,6)")
}
...
func TestSumPrecision(t *testing.T) {
	...
	assertIterScan(t, "[[15.299999999999999 15.299999999999999 {{false [153]} 1}]]", s, "SELECT sum(v1), sum(v2), sum(v3) from ks1.t1")
}
