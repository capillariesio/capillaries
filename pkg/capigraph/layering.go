...
func pushDownStressedByPriOrSecParent(nodeDefs []NodeDef, nodeLayerMap []int) bool {
	reScan := false
...
func pullDownIfThereIsRoom(nodeDefs []NodeDef, nodeLayerMap []int) bool {
	reScan := false
...
func buildLayerMap(nodeDefs []NodeDef) []int {
	...
	for {
		reScan := false
		if pushDownStressedByPriOrSecParent(nodeDefs, nodeLayerMap) {
			reScan = true
		}

		if pullDownIfThereIsRoom(nodeDefs, nodeLayerMap) {
			reScan = true
		}
